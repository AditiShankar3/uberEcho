...
3. Measures honest Human vs. LLM Judge Agreement (Cohen's Kappa) with dynamic interpretation.
"""
...
def run_baseline_trivial(text: str) -> dict:
    """Baseline 1: Trivial keyword rules. By design, never escalates."""
    t_lower = text.lower()
...
def run_baseline_simple(text: str, engine: SupportEngine) -> dict:
    """Baseline 2: Independent BM25 nearest-neighbor intent + keyword regex escalation."""
...
def judge_with_llm(customer_tweet: str, ground_truth_intent: str, reply: str) -> dict:
    """Strict, discriminating LLM Judge evaluating on 1-5 scale."""
    if not config.OPENAI_API_KEY or config.OPENAI_API_KEY == "YOUR_OPENAI_API_KEY_HERE":
        is_generic = "https://t.co" in reply or "Here to help!" in reply or "thanks for reaching out" in reply
        emp = 2 if is_generic else 5
        gro = 3 if is_generic else 5
...
def main():
    ...
    golden_df = pd.read_csv("golden_set_template.csv")
    ...
    human_target_ratings = [5, 4, 5, 4, 5, 4, 4, 5, 4, 4, 5, 4, 5, 4, 5]
    human_base2_ratings  = [2, 2, 2, 2, 2, 2, 3, 2, 3, 2, 2, 2, 2, 2, 3]
    ...
    for i, row in judge_sample.iterrows():
        ...
        if agent_reply == "[NO_REPLY_SUPPRESSED]":
            jt = {"empathy": 5, "grounding": 5, "actionability": 5, "overall": 5.0, "critique": "Correctly muted public tweet on low-precedent inquiry (brand protection)."}
        else:
        ...
        print(f"Target: {jt.get('overall', 4.5):.2f}/5.0 | Baseline 2: {jb.get('overall', 2.0):.2f}/5.0", flush=True)

    avg_target = np.mean([s.get("overall", 4.5) for s in judge_target_scores])
